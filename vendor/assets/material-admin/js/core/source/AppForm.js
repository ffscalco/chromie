(function(namespace, $) {
	...
	var AppForm = function() {
		...
		$(document).on('turbolinks:load', function() {
			...
			$("#participants-fields").on('cocoon:after-insert', function() {
				o.initialize();
			});
		});
	...
	p._initRadioAndCheckbox = function () {
		...
		$('.checkbox-styled input[type=checkbox], .radio-styled input[type=radio]').each(function () {
			if ($(this).next('span').length === 0) {
				$(this).after('<span>' +  $(this).attr("data-label") +'</span>');
			}
		});
	};
